...
function validateQuarter(slots, label, errors) {
  ...
  if (htMinutes > 0 && htMinutes < 120) {
    errors.push(`${label}: HT active only ${htMinutes} min/day, minimum is 120 min (2 h)`);
  }
    ...
    for (const [year, quarters] of Object.entries(data.tariffs)) {
      for (const [q, slots] of Object.entries(quarters)) {
        if (Array.isArray(slots)) {
          validateQuarter(slots, `${label}/${year}/${q}`, errors);
        }
      }
    }
  }
